...
function CustomerPage(){
    ...
    const onDelete = () => {
        ...
        request("customer/"+customer_id,"delete").then(res=>{
            var tmp_data = list.filter((item)=>item.customer_id != customer_id)
            setList(tmp_data)
        })
    }
    ...
    const onSave = () => {
        ...
        // case update
        if(item != null){
            param.customer_id = item.customer_id // add new key "category_id" to param
            method = "PUT"
            url = `customer/${param.customer_id}`
        }
    ...
    return(
        <div style={{padding:1}}>
            <div style={{padding:10,display:"flex",justifyContent:'space-between'}}>
                <div>Customer</div>
                <div>
                    <Button variant="primary" onClick={onShowModalForm}>Add Customer</Button>
                </div>
            </div>
           <Table striped bordered hover size='sm' responsive>
                ...
                {list?.map((item,index)=>{
                    return (
                        <tr key={index}>
                            ...
                            <td>{formatDateClient(item.updated_at)}</td>
                            { item.role_id == 1 ?  <td><Button size="sm" type="primary">Enable</Button></td> : <td><Button size="sm" type="primary" danger>Disable</Button></td> }                       
                            <td>
                                <Space>
            ...
            >
                <Modal show={showForm} onHide={onHideModalForm}>
                    <Modal.Header closeButton>
                        <Modal.Title>{item?.customer_id == null ? "Create" : "Update"}</Modal.Title>
                    </Modal.Header>
                    <Modal.Body>
                        <Form>
                    ...
                    <Modal.Footer>
                        ...
                        <Button variant="primary" onClick={onSave}>{(item?.customer_id == null) ? "Save" : "Update"}</Button>
                    </Modal.Footer>
                </Modal>
            </div>
        </div>
